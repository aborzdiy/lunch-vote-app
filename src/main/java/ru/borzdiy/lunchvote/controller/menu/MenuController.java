...
import static ru.borzdiy.lunchvote.controller.menu.MenuController.ADMIN_REST_URL;
...
public class MenuController extends AbstractMenuController {
    ...
    public List<Menu> getRestaurantMenu(@PathVariable("restaurantId") int restaurantId) {
        return super.getRestaurantMenu(restaurantId);
    }

    @PostMapping(value = "/restaurant/{restaurantId}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Menu> createRestaurantMenu(@Validated @RequestBody MenuTo menuTo, @PathVariable("restaurantId") int restaurantId) {
        Menu created = super.create(menuTo, restaurantId);
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void update(@Validated MenuTo menuTo, @PathVariable("id") int id) throws BindException {
